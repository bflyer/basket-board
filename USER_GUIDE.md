...
点击顶部 **战术库**，可以查看默认战术、默认练习和当前浏览器中的自定义战术。战术库分成三条横向栏目，卡片较多时可以在栏目内左右滑动，也可以点击栏目右侧的左右箭头。

![战术库实际界面](docs/assets/user-guide/tactic-library-real.png)

### 4.1 战术区

当前内置了拳头（弧顶双掩护）、剪刀（交叉双掩护）和帕子三个默认战术。每张卡片包含基础描述、步骤数、作者和标签。
...
练习区内置四个三人全场配合练习：直接上篮、手递手反跑、双手递手下顺，以及突分配合无球掩护内切。它们和战术区一样可以加载到战术板或导出 JSON，但不会混在战术区里。
...
| 战术库 | 打开战术区、练习区和自定义战术区 | 播放期间不能打开；卡片多时可横向滚动 |

...
4. 在下方 **步骤注释（按 Enter 换行）** 中输入本步讲解，例如“4号上提，为1号建立右侧挡拆角度”。需要分行时直接按 **Enter**；播放字幕会保留你输入的换行，并在屏幕宽度不足时自动继续换行。
5. 点击 **完成步骤**，进入下一步。
6. 重复以上操作，直到战术完成。
...
如果正在修改或检查某一个步骤，点击 **播放当前步骤**。它会从该步骤开始前的正确站位播放这一段，结束后仍停留在当前步骤，不会从第 1 步开始，也不会推进“逐步播放”的进度。初始摆放或当前空步骤没有可播放内容时，该按钮会保持灰色。

![“播放当前步骤”正在运行的实际界面](docs/assets/user-guide/playback-current-running.png)
...
点击顶部 **战术库**，可以查看默认战术、默认练习和当前浏览器中的自定义战术。战术区再分为进攻版块和联防版块；每个版块保持一条横向卡片栏，卡片较多时可以左右滑动，也可以点击右侧箭头。
...
- **进攻版块**：拳头（弧顶双掩护）、剪刀（交叉双掩护）和帕子。
- **联防版块**：122 全场紧逼和 212 半场联防。

每张卡片包含基础描述、步骤数、作者和标签。
...
![已载入默认战术模板](docs/assets/user-guide/loaded-template.png)

### 4.2 练习区

练习区内置五个练习：四个三人全场配合练习（直接上篮、手递手反跑、双手递手下顺、突分配合无球掩护内切），以及夹击挡拆五人轮转练习。它们和战术区一样可以加载到战术板或导出 JSON，但不会混在战术区里。

练习数量增加后，卡片不会把弹窗撑得越来越宽：每条栏目保持一行，使用触摸板、触摸屏、水平滚轮或左右箭头查看后面的内容。
...
| 播放当前步骤 / 停止播放 | 只播放正在编辑或查看的这一步 | 不从头开始，也不推进逐步播放进度 |
| 速度 | 调整播放和录制速度 | 播放过程中也能调整 |
| 战术库 | 打开进攻、联防、练习和自定义战术版块 | 播放期间不能打开；卡片多时可横向滚动 |

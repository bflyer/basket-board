...
面向使用者的完整图文说明：[篮球战术板使用说明](USER_GUIDE.md)
...
- 初始站位、多步骤跑位、轨迹与步骤注释
- 分栏战术库：进攻战术、联防战术、练习和保存到浏览器/用户文件夹的自定义战术，卡片支持横向滚动
- 连续播放、逐步播放和播放当前步骤，播放中实时调速、随时停止、撤销与分级重置
- 战术 JSON 导入/导出
...
公开模板位于 `tactics/`，`index.json` 保存模板清单。索引中的 `category` 为 `tactic`（战术区）或 `exercise`（练习区）；战术区再通过 `section` 的 `offense` 和 `zone-defense` 分到进攻、联防版块。每个战术使用独立 JSON 文件。模板坐标采用相对于球场宽高的归一化坐标：左上角是 `(0, 0)`，右下角是 `(1, 1)`，因此可以在不同尺寸的浏览器中按比例还原。
